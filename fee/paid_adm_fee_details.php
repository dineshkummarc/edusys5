...
function printDiv(income) {
     ...
     document.body.innerHTML = originalContents;
		...
		if((isset($_GET['class']))&&($_GET['academic_year'])&&($_GET['from'])&&($_GET['to']))
		{
			...
			$from=$_GET["from"];
			$to=$_GET["to"];
			$sql_tot="select sum(tot_paid) as total_paid_fee from student_adm_fee where class='".$class."' and academic_year='".$academic_year."' and (rec_date BETWEEN '$from' and '$to')";
		    $sql_set_fee="select adm_fee from set_adm_fee where class='".$class."' and academic_year='".$academic_year."'";	
		...
		
		{
			...
			$sql_tot="select sum(tot_paid) as total_paid_fee from student_adm_fee where class='".$class."' and academic_year='".$academic_year."'";
		    $sql_set_fee="select adm_fee from set_adm_fee where class='".$class."' and academic_year='".$academic_year."'";	
	...
	<?php
		...
		$sql="select id,roll_no,name,class,academic_year,rec_date,sum(adm_fee) as paid_adm_fee from student_adm_fee where class='".$class."' and academic_year='".$academic_year."' and (rec_date BETWEEN '$from' and '$to') group by roll_no ORDER BY name LIMIT $start_from, $num_rec_per_page ";
		$sql_set_fee="select id,adm_fee from set_adm_fee where class='".$class."' and academic_year='".$academic_year."' ORDER BY id DESC LIMIT $start_from, $num_rec_per_page ";
		}
		...
		$sql="select id,roll_no,name,class,academic_year,rec_date,sum(adm_fee) as paid_adm_fee from student_adm_fee where class='".$class."' and academic_year='".$academic_year."' group by roll_no ORDER BY name LIMIT $start_from, $num_rec_per_page ";
		$sql_set_fee="select id,adm_fee from set_adm_fee where class='".$class."' and academic_year='".$academic_year."' ORDER BY id DESC LIMIT $start_from, $num_rec_per_page ";
		}
	...
	if((isset($_GET['class']))&&(isset($_GET['academic_year'])))
		{
			$class=$_GET["class"];
			$academic_year=$_GET["academic_year"];
		$sql = "SELECT * FROM student_adm_fee where academic_year='".$academic_year."' and class='".$class."' and (rec_date BETWEEN '$from' and '$to')"; 
		}
		if(isset($_GET['class']))
		{
			$class=$_GET["class"];
			...
			$sql = "SELECT * FROM student_adm_fee where class='".$class."' and (rec_date BETWEEN '$from' and '$to')"; 
			}
		...
		if(isset($_GET['academic_year']))
		{
			...
			$sql = "SELECT * FROM student_adm_fee where academic_year='".$academic_year."' and (rec_date BETWEEN '$from' and '$to')"; 
			}
...
<!---------//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////----------->  
			...
			<div class="control-group">
				...
				$sql_tot="select sum(tot_paid) as tot_paid_fee from student_adm_fee ";
				$result_tot=mysqli_query($conn,$sql_tot);
				if($row_tot=mysqli_fetch_array($result_tot,MYSQLI_ASSOC))
				{
...
$today_date=date("Y-m-d");
		...
		$sql="select id,roll_no,name,class,academic_year,rec_date,sum(adm_fee) as paid_adm_fee from student_adm_fee where rec_date='".$today_date."' group by roll_no ORDER BY name LIMIT $start_from, $num_rec_per_page ";
		$result=mysqli_query($conn,$sql);
	...
	<?php 
	$sql = "SELECT * FROM student_adm_fee"; 
	$result = mysqli_query($conn,$sql); //run the query
	$total_records = mysqli_num_rows($result);  //count number of records
	$total_pages = ceil($total_records / $num_rec_per_page); 
