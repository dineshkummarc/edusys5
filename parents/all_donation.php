...
function printDiv(income) {
     ...
     document.body.innerHTML = originalContents;
					...
					<form class="form-inline" action="all_donation.php" method="get">
					 ...
					 <select class="form-control" name="towards">
						<?php
						 ...
						 $sql_name="select distinct towards from anv_don";
		...
		{
		$sql_tot="select towards,sum(amount)as tot_amount from anv_don where towards='".$towards."' and (rec_date BETWEEN '$from' and '$to')";	
		}
		if(isset($_GET['towards']))
		{
			if((($_GET['from'])=="")&&(($_GET['to'])==""))
			{
			$sql_tot="select towards,sum(amount)as tot_amount from anv_don where towards='".$towards."'";
			}
		}
		if((isset($_GET['from']))&&(isset($_GET['to']))){
			if((($_GET['towards']))==""){
				$sql_tot="select sum(amount)as tot_amount from anv_don where (rec_date BETWEEN '$from' and '$to')";
			}
		}
	...
	<?php
		...
		if((isset($_GET['towards']))&&(isset($_GET['from']))&&(isset($_GET['to'])))
		{
		$sql="select * from anv_don where towards='".$towards."' and (rec_date BETWEEN '$from' and '$to') ORDER BY id DESC LIMIT $start_from, $num_rec_per_page ";
		}
		if(isset($_GET['towards']))
		{
			...
			if((($_GET['from'])=="")&&(($_GET['to'])==""))
				{
				$sql="select * from anv_don where towards='".$towards."' ORDER BY id DESC LIMIT $start_from, $num_rec_per_page ";
				}
		}
		if((isset($_GET['from']))&&(isset($_GET['to']))){
			if((($_GET['towards']))==""){
				$sql="select * from anv_don where (rec_date BETWEEN '$from' and '$to') ORDER BY id DESC LIMIT $start_from, $num_rec_per_page ";
			}
		}
	...
	if((isset($_GET['towards']))&&(isset($_GET['from']))&&(isset($_GET['to'])))
		{
		$sql = "SELECT * FROM anv_don where towards='".$towards."' and (rec_date BETWEEN '$from' and '$to')"; 
		}
		if(isset($_GET['towards']))
		{
		...
		if((($_GET['from'])=="")&&(($_GET['to'])==""))
			{
			$sql = "SELECT * FROM anv_don where towards='".$towards."'"; 
			}
		...
		if((isset($_GET['from']))&&(isset($_GET['to']))){
			if((($_GET['towards']))=="")
			{
				$sql = "SELECT * FROM anv_don where (rec_date BETWEEN '$from' and '$to')"; 
			}
		}
...
<!---------//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////----------->  
					...
					?></div>
					  ...
					  <input type="submit" class="btn btn-primary w3-card-4" name="filter" value="Filter">
					   ...
					   <a href="<?php echo 'all_donation.php?edit='.'yes';?>"><button type="button"  class="btn btn-primary btn-md ">Edit Fee</button></a>
						...
						<?php 
				$sql_tot="select sum(amount)as tot_amount from anv_don";
				$result_tot=mysqli_query($conn,$sql_tot);
				if($row_tot=mysqli_fetch_array($result_tot,MYSQLI_ASSOC))
				{
	...
	<?php
		...
		if (isset($_GET["page"])) { $page  = $_GET["page"]; } else { $page=1; }; 
		$start_from = ($page-1) * $num_rec_per_page; 
		$sql="select * from anv_don ORDER BY id DESC LIMIT $start_from, $num_rec_per_page";	
		$result=mysqli_query($conn,$sql);
		$row_count =1;
