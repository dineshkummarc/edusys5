...
<div class="container-fluid">
  ...
  <div class="row">
	...
	$sql="select id,first_name,roll_no,class_join,present_class,academic_year,class_stream,village,dob,join_date,photo_name,photo_path,photo_type from students where present_class='".$filt_class."' ORDER BY id DESC LIMIT $start_from, $num_rec_per_page";
	}
	
	}
	else{
		
	$sql="select id,first_name,roll_no,present_class,class_stream,academic_year,class_join,village,dob,join_date,photo_name,photo_path,photo_type from students ORDER BY id DESC LIMIT $start_from, $num_rec_per_page";	
	}
	...
	?>
        ...
        <a href="<?php echo 'upd_register.php?id='.$row['id']; ?>"><button type="button" class="btn btn-primary">Edit</button></a>
	...
	if(isset($_GET["filt_submit"]))
	{
		...
		{
			$filt_class=$_GET["filt_class"];
			$sql="select id,first_name,roll_no,class_join,present_class,class_stream,village,dob,join_date,photo_name,photo_path,photo_type from students where present_class='".$filt_class."' ORDER BY id DESC LIMIT $start_from, $num_rec_per_page";
		$result=mysqli_query($conn,$sql);
		$total_students=mysqli_num_rows($result);
	...
	$sql="select id,first_name,roll_no,present_class,class_stream,class_join,village,dob,join_date,photo_name,photo_path,photo_type from students";
	$result=mysqli_query($conn,$sql);
