...
function printDiv(income) {
	...
	<?php
		...
		$sql="select roll_no,name,class,academic_year,rec_date,sum(adm_fee) as paid_adm_fee from student_fee where class='".$class."' and academic_year='".$academic_year."' group by roll_no ORDER BY id DESC LIMIT $start_from, $num_rec_per_page ";
		
		$sql_set_fee="select adm_fee from set_fee where class='".$class."' and academic_year='".$academic_year."' ORDER BY id DESC LIMIT $start_from, $num_rec_per_page ";
		}
...
<!---------//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////----------->  
	...
	<?php
		...
		if (isset($_GET["page"])) { $page  = $_GET["page"]; } else { $page=1; }; 
		$start_from = ($page-1) * $num_rec_per_page; 
		$sql="select * from student_fee ORDER BY id DESC LIMIT $start_from, $num_rec_per_page";	
		$result=mysqli_query($conn,$sql);
		$row_count =1;
