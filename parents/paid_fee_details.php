...
function printDiv(income) {
     ...
     document.body.innerHTML = originalContents;
		...
		if((isset($_GET['class']))&&($_GET['academic_year'])&&($_GET['from'])&&($_GET['to']))
		{
			...
			$from=$_GET["from"];
			$to=$_GET["to"];
			$sql_tot="select sum(tot_paid) as total_paid_fee from student_fee where class='".$class."' and academic_year='".$academic_year."' and (rec_date BETWEEN '$from' and '$to')";
		...
		
		{
			...
			$sql_tot="select sum(tot_paid) as total_paid_fee from student_fee where class='".$class."' and academic_year='".$academic_year."'";
		    $sql_set_fee="select adm_fee from set_fee where class='".$class."' and academic_year='".$academic_year."'";	
	...
	<?php
		...
		$sql="select id,roll_no,name,class,academic_year,rec_date,sum(adm_fee) as paid_adm_fee from student_fee where class='".$class."' and academic_year='".$academic_year."' and (rec_date BETWEEN '$from' and '$to') group by roll_no ORDER BY id DESC LIMIT $start_from, $num_rec_per_page ";
		...
		$sql="select id,roll_no,name,class,academic_year,rec_date,sum(adm_fee) as paid_adm_fee from student_fee where class='".$class."' and academic_year='".$academic_year."' group by roll_no ORDER BY id DESC LIMIT $start_from, $num_rec_per_page ";
		$sql_set_fee="select id,adm_fee from set_fee where class='".$class."' and academic_year='".$academic_year."' ORDER BY id DESC LIMIT $start_from, $num_rec_per_page ";
		}
...
$tot_adm_fee=$row_set_fee["adm_fee"];
	...
	if((isset($_GET['class']))&&(isset($_GET['academic_year'])))
		...
		if(isset($_GET['class']))
		{
			...
			$sql = "SELECT * FROM student_fee where class='".$class."' and (rec_date BETWEEN '$from' and '$to')"; 
			}
...
$today_date=date("Y-m-d");
		...
		$sql="select id,roll_no,name,class,academic_year,rec_date,sum(adm_fee) as paid_adm_fee from student_fee where rec_date='".$today_date."' group by roll_no ORDER BY id DESC LIMIT $start_from, $num_rec_per_page ";
		$result=mysqli_query($conn,$sql);
