...
function printDiv(income) {
	...
	<?php
		...
		if((isset($_GET['from']))&&!empty($_GET['to']))
		{
			...
			$sql_amount="select * from income where  academic_year='".$academic_year."' and (rec_date BETWEEN '$from' and '$to') ORDER BY rec_date desc LIMIT $start_from, $num_rec_per_page ";
		   
		   
		   $sql_tot="select sum(amount) as total_amount from income where  academic_year='".$academic_year."' and (rec_date BETWEEN '$from' and '$to') ORDER BY rec_date desc LIMIT $start_from, $num_rec_per_page";
			//var_dump($sql_tot);
		}
		...
		else
		{
			...
			$sql_amount="select * from income where  academic_year='".$academic_year."' ORDER BY rec_date desc LIMIT $start_from, $num_rec_per_page ";
			
			$sql_tot="select sum(amount) as total_amount from income where academic_year='".$academic_year."' ORDER BY rec_date desc LIMIT $start_from, $num_rec_per_page";
	...
	<?php
	$sql = "SELECT * FROM income where academic_year='".$academic_year."'"; 
