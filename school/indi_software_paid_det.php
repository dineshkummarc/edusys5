...
function printDiv(income) {
     ...
     document.body.innerHTML = originalContents;
			...
			<div class="container-fluid">
				...
				if(isset($_GET['filter'])){
					...
					?>	
						...
						<form class="form-inline" action="indi_software_paid_det.php" method="get">
		...
		$sql="select * from student_software_fee where academic_year='".$cur_academic_year."' and (rec_date BETWEEN '$from' and '$to') and name='".$first_name."' and roll_no='".$roll_no."'";
		...
		$roll_no=$_GET['roll_no'];
		//$present_class=$_GET['present_class'];
		$sql="select * from student_software_fee where academic_year='".$cur_academic_year."' and name='".$first_name."' and roll_no='".$roll_no."'";
	...
	if((isset($_GET['from']))&&(isset($_GET['to'])))
		...
		$sql_tot="select sum(adm_fee) as paid_fee_tot from student_software_fee where academic_year='".$cur_academic_year."' and (rec_date BETWEEN '$from' and '$to') and name='".$first_name."' and roll_no='".$roll_no."'";
       		
		}
		else
		{
		
		$sql_tot="select sum(adm_fee) as paid_fee_tot from student_software_fee where academic_year='".$cur_academic_year."' and name='".$first_name."' and roll_no='".$roll_no."'";
	...
	$sql_fee_tot="select adm_fee  from set_software_fee where academic_year='".$cur_academic_year."' and class='".$class."' order by id desc limit 1";
	...
	?>
				...
				<a href="<?php echo 'student_software_fee.php?id='.$row_tot["id"].'&edit=yes&name_edit='.$row_tot["name"].'&roll_no_edit='.$row_tot["roll_no"].'&class_edit='.$row_tot["class"];?>">  <i class="fa fa-pencil-square-o fa-lg" aria-hidden="true"></i></a>
				<a href="<?php echo 'delete_software_fee_paid.php?id='.$row_tot["id"];?>">  <i class="fa fa-trash-o fa-lg" style="color:red;" aria-hidden="true"></i></a>
			   </div></td>
				</tr>
