...
function printDiv(income) {
     ...
     document.body.innerHTML = originalContents;
		...
		$sql="select * from student_van_fee where academic_year='".$cur_academic_year."' and (rec_date BETWEEN '$from' and '$to') and first_name='".$first_name."' and roll_no='".$roll_no."'";
		...
		$roll_no=$_GET['roll_no'];
		//$present_class=$_GET['present_class'];
		$sql="select * from student_van_fee where academic_year='".$cur_academic_year."' and first_name='".$first_name."' and roll_no='".$roll_no."'";
	...
	if((isset($_GET['from']))&&(isset($_GET['to'])))
		...
		$sql_tot="select sum(van_fee) as paid_fee_tot from student_van_fee where academic_year='".$cur_academic_year."' and (rec_date BETWEEN '$from' and '$to') and first_name='".$first_name."' and roll_no='".$roll_no."'";
       		
		}
		else
		{
		
		$sql_tot="select sum(van_fee) as paid_fee_tot from student_van_fee where academic_year='".$cur_academic_year."' and first_name='".$first_name."' and roll_no='".$roll_no."'";
	...
	$sql_fee_tot="select van_fee  from set_van_fee where academic_year='".$cur_academic_year."' and route_name='".$route_name."' and stage_name='".$stage_name."' order by id desc limit 1";
	...
	{
		$paid_fee_total=$row_fee_tot["van_fee"];
	}
	...
	?>
				...
				<td style="text-align:center;"><?php echo $row_tot["van_fee"];?></td>
				<td><div class="btn-group">
				<a href="<?php echo 'student_van_fee.php?id='.$row_tot["id"].'&edit=yes&name_edit='.$row_tot["name"].'&roll_no_edit='.$row_tot["roll_no"].'&class_edit='.$row_tot["class"];?>">  <i class="fa fa-pencil-square-o fa-lg" aria-hidden="true"></i></a>
				<a href="<?php echo 'delete_van_fee_paid.php?id='.$row_tot["id"];?>">  <i class="fa fa-trash-o fa-lg" style="color:red;" aria-hidden="true"></i></a>
			   </div></td>
				</tr>
