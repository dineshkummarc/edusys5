...
if(isset($_GET['month_att_sms'])){
  ...
  if(mysqli_num_rows($result_student_att)>0)
  {
    foreach($result_student_att as $row_att){
      ...
      $sql="insert into individual_notifications (title,details,student_id,academic_year,indi_viewed) values('$subject','$noti_message','$student_id','$cur_academic_year','False')";
      $conn->query($sql);
